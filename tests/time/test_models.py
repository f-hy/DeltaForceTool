...
def test_timeconfig_custom():
  ...
  config = TimeConfig(timezone="UTC", update_interval_ms=500)
  assert config.timezone == "UTC"
  assert config.update_interval_ms == 500
def test_timeevent_creation():
  """Test TimeEvent creation."""
  now = datetime.now()
  event = TimeEvent(timestamp=now.timestamp(), current_time=now, formatted_time="12:34:56", timezone="Asia/Shanghai")
