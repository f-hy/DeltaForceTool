...
def load_config():
  ...
  default_config = {"hotkeys": {"ocr_trigger": "alt+s", "exit": "ctrl+alt+q"}, "ocr_settings": {"overlay_alpha": 0.7, "rect_color": "#00ff00", "rect_width": 3, "rect_dash": [4, 2]}}
...
def run_main_app_with_ocr_shortcut():
  ...
  label = tk.Label(main_window, text="DeltaForceTool\nPress {} for OCR".format(ocr_key.upper()), font=("Consolas", 10))
  label.pack(pady=20)

  info = tk.Label(main_window, text="TimeClock: uv run main.py clock", font=("Consolas", 8), fg="gray")
  info.pack(pady=10)

  status = tk.Label(main_window, text="Status: Initializing...", font=("Consolas", 8), fg="orange")
  status.pack(pady=10)
