...
class TimeClock(Singleton, Subject):
  ...
  def _update(self) -> None:
    ...
    event = TimeEventData(timestamp=now.timestamp(), current_time=now, formatted_time=time_str, timezone=self._config.timezone)
    self.notify(event)
...
class TkinterTimeDisplay(Observer):
  ...
  def _setup_ui(self) -> None:
    ...
    self.label = tk.Label(self.root, text="", font=("Consolas", 32, "bold"), fg="#00ff00", bg=self.transparent_color)
